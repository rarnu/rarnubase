...
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey128; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey192; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey256; Dest: TStream); overload;
procedure EncryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256; Dest: TStream); overload;
...
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey128; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey128; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey192; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey192; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const Key: TAESKey256; Dest: TStream); overload;
procedure DecryptAESStreamECB(Source: TStream; Count: cardinal; const ExpandedKey: TAESExpandedKey256; Dest: TStream); overload;
...
function _AESEncryptECB(key: string; keyType: Integer; ex: Boolean; src: string): string;
function _AESEncryptCBC(v: string; key: string; keyType: Integer; ex: Boolean; src: string): string;
function _AESDecryptECB(key: string; keyType: Integer; ex: Boolean; src: string): string;
function _AESDecryptCBC(v: string; key: string; keyType: Integer; ex: Boolean; src: string): string;
...
function GenerateVector(v: string): TAESBuffer;
var
  vec: TAESBuffer;
begin
  FillChar(vec, SizeOf(vec), 0);
  Move(PChar(v)^, vec, Min(SizeOf(vec), Length(v)));
  Result := vec;
end;

function GenerateKey128(key: string): TAESKey128;
var
  k: TAESKey128;
begin
  FillChar(k, SizeOf(k), 0);
  Move(PChar(key)^, k, Min(SizeOf(k), Length(key)));
  Result := k;
end;

function GenerateKey192(key: string): TAESKey192;
var
  k: TAESKey192;
begin
  FillChar(k, SizeOf(k), 0);
...
function GenerateKey256(key: string): TAESKey256;
var
  k: TAESKey256;
begin
  FillChar(k, SizeOf(k), 0);
...
function GenerateKey128Enc(key: string): TAESExpandedKey128;
var
  k: TAESKey128;
  ek: TAESExpandedKey128;
begin
  FillChar(k, SizeOf(k), 0);
  Move(PChar(key)^, k, Min(SizeOf(k), Length(key)));
  FillChar(ek, SizeOf(ek), 0);
  ExpandAESKeyForEncryption(k, ek);
  Result := ek;
end;

function GenerateKey192Enc(key: string): TAESExpandedKey192;
var
  k: TAESKey192;
  ek: TAESExpandedKey192;
begin
  FillChar(k, SizeOf(k), 0);
  Move(PChar(key)^, k, Min(SizeOf(k), Length(key)));
  FillChar(ek, SizeOf(ek), 0);
...
function GenerateKey256Enc(key: string): TAESExpandedKey256;
var
  k: TAESKey256;
  ek: TAESExpandedKey256;
begin
  FillChar(k, SizeOf(k), 0);
  Move(PChar(key)^, k, Min(SizeOf(k), Length(key)));
  FillChar(ek, SizeOf(ek), 0);
...
function GenerateKey128Dec(key: string): TAESExpandedKey128;
var
  k: TAESKey128;
  ek: TAESExpandedKey128;
begin
  ...
  Move(PChar(key)^, k, Min(SizeOf(k), Length(key)));
  FillChar(ek, SizeOf(ek), 0);
  ExpandAESKeyForDecryption(k ,ek);
  Result := ek;
end;

function GenerateKey192Dec(key: string): TAESExpandedKey192;
var
  k: TAESKey192;
  ek: TAESExpandedKey192;
begin
...
function GenerateKey256Dec(key: string): TAESExpandedKey256;
var
  k: TAESKey256;
  ek: TAESExpandedKey256;
begin
...
var
  ...
  sDest: TStringStream;
  key128: TAESKey128;
  key128Ex: TAESExpandedKey128;
  key192: TAESKey192;
  key192Ex: TAESExpandedKey192;
  key256: TAESKey256;
  key256Ex: TAESExpandedKey256;
begin
  Result := '';
  sSrc := TStringStream.Create(src);
  sDest := TStringStream.Create('');
  try
    case keyType of
      128: begin
        if ex then begin
          key128Ex := GenerateKey128Enc(key);
          EncryptAESStreamECB(sSrc, 0, key128Ex, sDest);
        end else begin
          key128 := GenerateKey128(key);
          EncryptAESStreamECB(sSrc, 0, key128, sDest);
        end;
      end;
      192:begin
        if ex then begin
          key192Ex := GenerateKey192Enc(key);
          EncryptAESStreamECB(sSrc, 0, key192Ex, sDest);
        end else begin
          key192 := GenerateKey192(key);
          EncryptAESStreamECB(sSrc, 0, key192, sDest);
        end;
      end;
      256:begin
        if ex then begin
          key256Ex := GenerateKey256Enc(key);
          EncryptAESStreamECB(sSrc, 0, key256Ex, sDest);
        end else begin
          key256 := GenerateKey256(key);
          EncryptAESStreamECB(sSrc, 0, key256, sDest);
        end;
      end;
    end;
    Result := StringToHex(sDest.DataString);
  finally
    sSrc.Free;
    sDest.Free;
  end;
...
begin
  ...
  sSrc := TStringStream.Create(src);
  sDest := TStringStream.Create('');
  vector := GenerateVector(v);
  try
    case keyType of
      128: begin
        if ex then begin
          key128Ex := GenerateKey128Enc(key);
          EncryptAESStreamCBC(sSrc, 0, key128Ex, vector, sDest);
        end else begin
          key128 := GenerateKey128(key);
          EncryptAESStreamCBC(sSrc, 0, key128, vector, sDest);
        end;
      end;
      192:begin
        if ex then begin
          key192Ex := GenerateKey192Enc(key);
          EncryptAESStreamCBC(sSrc, 0, key192Ex, vector, sDest);
        end else begin
          key192 := GenerateKey192(key);
          EncryptAESStreamCBC(sSrc, 0, key192, vector, sDest);
        end;
      end;
      256:begin
        if ex then begin
          key256Ex := GenerateKey256Enc(key);
          EncryptAESStreamCBC(sSrc, 0, key256Ex, vector, sDest);
        end else begin
          key256 := GenerateKey256(key);
          EncryptAESStreamCBC(sSrc, 0, key256, vector, sDest);
        end;
      end;
    end;
    Result := StringToHex(sDest.DataString);
...
begin
  ...
  sSrc := TStringStream.Create(src);
  sDest := TStringStream.Create('');
  sSrc := TStringStream.Create(HexToString(src));
  sDest := TStringStream.Create('');
  try
    case keyType of
      128: begin
        if ex then begin
          key128Ex := GenerateKey128Dec(key);
          DecryptAESStreamECB(sSrc, 0, key128Ex, sDest);
        end else begin
          key128 := GenerateKey128(key);
          DecryptAESStreamECB(sSrc, 0, key128, sDest);
        end;
      end;
      192: begin
        if ex then begin
          key192Ex := GenerateKey192Dec(key);
          DecryptAESStreamECB(sSrc, 0, key192Ex, sDest);
        end else begin
          key192 := GenerateKey192(key);
          DecryptAESStreamECB(sSrc, 0, key192, sDest);
        end;
      end;
      256: begin
        if ex then begin
          key256Ex := GenerateKey256Dec(key);
          DecryptAESStreamECB(sSrc, 0, key256Ex, sDest);
        end else begin
          key256 := GenerateKey256(key);
          DecryptAESStreamECB(sSrc, 0, key256, sDest);
        end;
      end;
    end;
  ...
  try
    case keyType of
      128: begin
        if ex then begin
          key128Ex := GenerateKey128Dec(key);
          DecryptAESStreamCBC(sSrc, 0, key128Ex, vector, sDest);
        end else begin
          key128 := GenerateKey128(key);
          DecryptAESStreamCBC(sSrc, 0, key128, vector, sDest);
        end;
      end;
      192: begin
        if ex then begin
          key192Ex := GenerateKey192Dec(key);
          DecryptAESStreamCBC(sSrc, 0, key192Ex, vector, sDest);
        end else begin
          key192 := GenerateKey192(key);
          DecryptAESStreamCBC(sSrc, 0, key192, vector, sDest);
        end;
      end;
      256: begin
        if ex then begin
          key256Ex := GenerateKey256Dec(key);
          DecryptAESStreamCBC(sSrc, 0, key256Ex, vector, sDest);
        end else begin
          key256 := GenerateKey256(key);
          DecryptAESStreamCBC(sSrc, 0, key256, vector, sDest);
        end;
      end;
    end;
    Result := sDest.DataString;
